package me.ufo.factions.extras.tools.tray;
...
public final class TrayPickaxe {
  ...
  public void onBlockBreak(final Player player, final Location location) {
    ...
    final Location neg = location.clone().subtract(nx, 0, nz);
    final Location pos = location.clone().add(px, 0, pz);
    final Cuboid cuboid = new Cuboid(neg, pos);

    for (final Block block : cuboid.getBlocksByMaterial(Material.NETHERRACK)) {
  ...
  public void onBlockBreak(final BlockBreakEvent event) {
    ...
    if (!Item.hasKey(item, "TRAY_PICKAXE")) {
      return;
    }
